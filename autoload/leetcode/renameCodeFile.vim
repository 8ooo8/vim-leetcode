...
fu! leetcode#renameCodeFile#renameCurrentCodeFile(newname)
  ...
  for c in code_filenames
    if c == a:newname || c == a:newname .'.' .g:leetcode_lang
  ...
  let new_code_filepath = dir_path .g:leetcode_path_delimit .a:newname .'.' .g:leetcode_lang 
  "" :saveas saves the marks, etc, as well
  exe 'sil sav ' .fnameescape(new_code_filepath)
  sil bw! #
  exe 'sil !rm "' .old_code_filepath .'"'
  echo 'Renamed from "' .old_code_filename .'" to "' .a:newname .'.' .g:leetcode_lang .'".'
  retu 0
endfu
